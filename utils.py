...
def compute_P(n, type, args=None):
    if type == 'cycle':
        ...
        D_sqrt_inv = np.zeros((n-1, n-1), dtype=float)
        for i in range(n-1):
            for j in range(1,2):
                A[i][(i+j)%(n-1)] = 1
                A[i][(i-j)%(n-1)] = 1
            A[i][i] = 0
        delta = [A[0][j] for j in range(n-1)].count(1)
        for i in range(n-1):
            D_sqrt_inv[i][i] = 1/np.sqrt(delta)
        Lap = np.eye(n-1, dtype=float) - np.dot(np.dot(D_sqrt_inv, A), D_sqrt_inv)
        P_tmp = np.eye(n-1, dtype=float) - delta/(delta+1)*Lap
        for i in range(n-1):
            for j in range(n-1):
                B[i][j] = P_tmp[i][j]
        B[n-1][n-1] = 1 
        ...
        P_tmp = np.eye(n, dtype=float) - delta/(delta+1)*Lap
        P = P_tmp
